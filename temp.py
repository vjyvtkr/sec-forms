...
for i in inp.find_all():
    ...
    if check in i.attrs:
        ...
        rows.append(ud.unidecode(fin_text))
        fin_text_list.append(ud.unidecode(fin_text))
...
'''
df = pd.read_csv('C:\\Users\\u505123\\Documents\\Project\\Dataset\\10_K\\outt2101.csv', ',')
facts = list(df['fact'])
sentences = []
result = []
...
for i in facts:
    ...
    #t_fact = re.sub(r'<div\b[^>]*>(.*?)</div>|<p\b[^>]*>(.*?)</p>','#',str(i))    
    t_fact = t_fact.replace('&nbsp;', ' ')
    t_fact = re.sub(r'\s+',' ', t_fact)
    final_fact = ud.unidecode(t_fact)
    sentences.append(final_fact)
    t_result = re.findall(r'\. [A-Z][^\.]*[0-9]+\.?[0-9]+?%[^\.]*\.[A-Z\.| a-z]*[^\.]*[\. A-Z|\.$]', str(final_fact))
    #t_result = re.findall(r'\. [A-Z0-9].*?percent.*?\.', str(final_fact))
    if len(t_result)>0:
        for j in t_result:
            if 'sale' in j:
                result.append(j)
